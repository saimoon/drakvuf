...
static bool
add_trace_points(proctracer *p, drakvuf_t drakvuf, vmi_instance_t vmi, addr_t process, addr_t cr3, char *proc_name)
{
    ...
    while (traps_ok)
    {
        ...
        unicode_string_t *us = vmi_read_unicode_str_va(vmi, next_module + p->offsets[LDR_DATA_TABLE_ENTRY_BASEDLLNAME],
                                                       pid);
            ...
            if (VMI_SUCCESS == status)
            {
                ...
                if (p->mod_config.find((char *) out.contents) != p->mod_config.end())
                {
                    ...
                    for (addr_t off: p->mod_config[(char *) out.contents])
                    {
                        ...
                        addr_t trap_pa;
                        if ( VMI_FAILURE == vmi_pagetable_lookup(vmi, cr3, dllbase + off, &trap_pa) )
                        {
                            printf("[PROCTRACER] vmi_pagetable_lookup() Failed\n");
                            return false;
                        }
...
static event_response_t cb(drakvuf_t drakvuf, drakvuf_trap_info_t *info)
{
    ...
    if (p->mod_config.find(proc_name) == p->mod_config.end() ||
        p->trace_status.find(info->regs->cr3) != p->trace_status.end())
    {
        free(proc_name);
        drakvuf_release_vmi(drakvuf);
        return 0;
...
proctracer::proctracer(drakvuf_t drakvuf, const void *config, output_format_t output)
{
    ...
    for (int i = 0; i < conf_modules_len; i++)
    {
        ...
        printf("[PROCTRACER] Module conf: %s --> count: %d\n", mod_path, symbols->count);
    ...
    for (int i = 0; i < conf_modules_len; i++)
    { // We need a different loop so DLLs of already running processes can be traced
        char *mod_name = (char *) json_object_get_string(json_object_array_get_idx(conf_modules, i));
        addr_t cur_proc;
        if (drakvuf_find_process(drakvuf, 0, mod_name, &cur_proc))
        {
    ...
    if ( !drakvuf_get_function_rva(rekall_profile, "PsGetCurrentThreadTeb", &this->trap.breakpoint.rva))
    {
        printf("[PROCTRACER] Can't find PsGetCurrentThreadTeb!\n");
        throw -1;
    }

    if ( !drakvuf_get_function_rva(rekall_profile, "PspExitProcess", &this->exit_trap.breakpoint.rva))
    {
